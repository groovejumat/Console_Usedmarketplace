...
class DbHandler {
    ...
    // fetching single user by id 유저에 대한 모든 정보를 가지고 온다. user_name값을 기반으로 정보를 찾는다. //임시 방편 일수도 있다.
    public function getUserAsName($user_name) {
        $stmt = $this->conn->prepare("SELECT user_id, name, email, gcm_registration_id, created_at FROM users WHERE name = ?");
    ...
    //거래 정보를 db에 저장하는 기능을 담도록한다.
    public function addTradingproduct($sell_user_id, $buy_user_id, $product_id, $productprogress,$shippinginfo) {
        $response = array();

        //거래정보를 저장 판매자 유저 id, 구매자 유저 id, 제품 id, 제품진행상황(숫자형), 배송정보.
        $stmt = $this->conn->prepare("INSERT INTO tradingproduct (sell_user_id, buy_user_id, product_id, productprogress, shippingInfo) values(?, ?, ?, ?, ?)");
        $stmt->bind_param("iiiis", $sell_user_id, $buy_user_id, $product_id, $productprogress, $shippinginfo); //변수 값 연결.
        ...
        if ($result) {
            ...
            // get the message
            $tradingproduct_id = $this->conn->insert_id; //최근에 넣은 데이터 값의 primary값을 꺼내옴.
            $stmt = $this->conn->prepare("SELECT sell_user_id, buy_user_id, product_id, productprogress, shippingInfo FROM tradingproduct WHERE tradingproduct_id = ?");
            $stmt->bind_param("i", $tradingproduct_id);
            if ($stmt->execute()) {
                $stmt->bind_result($sell_user_id, $buy_user_id, $product_id, $productprogress, $shippinginfo); //결과 값을 바인딩
                $stmt->fetch();
                $tmp = array();
                $tmp['sell_user_id'] = $sell_user_id;
                $tmp['buy_user_id'] = $buy_user_id;
                $tmp['product_id'] = $product_id;
                $tmp['productprogress'] = $productprogress;
                $tmp['shippinginfo'] = $shippinginfo;
                $response['message'] = $tmp; //arraylist값을 넣는다.
            }
        } else {
            $response['error'] = true;
            $response['message'] = 'Failed add tradingproduct';
        }
    ...
    // messaging in a chat room / to persional message //이 함수를 호출시킨다.
    public function createChatroom($user_name, $writer_id, $product_id, $product_name,$product_writer) {
        $response = array(); //응답 배열 생성.

        //INSERT INTO `chat_rooms`(`name`, `product_id`) VALUES ('user3','16') 다음과 같은 방식으로 쿼리 처리 진행.
        $stmt = $this->conn->prepare("INSERT INTO chat_rooms (name, writerid, product_id,product_name,product_writer) values(?, ?, ?, ?, ?)"); //쿼리준비
        $stmt->bind_param("ssiss", $user_name, $writer_id, $product_id, $product_name, $product_writer); //값 연결
        ...
        if ($result) {
            ...
            $stmt = $this->conn->prepare("SELECT chat_room_id, name, writerid, product_id, product_name, product_writer, created_at FROM chat_rooms WHERE chat_room_id = ?");
            $stmt->bind_param("i", $chat_room_id);
            if ($stmt->execute()) { //해당 쿼리를 실행한 결과 값을 가지고 온다.
                $stmt->bind_result($chat_room_id, $name, $writerid, $product_id, $product_name, $product_writer, $created_at);
                $stmt->fetch();
                $tmp = array();
                $tmp['chat_room_id'] = $chat_room_id;
                $tmp['name'] = $name;
                $tmp['writerid'] = $writerid;
                $tmp['product_id'] = $product_id;
    ...
    // 현재 거래중인 제품중에서 판매중인 제품들을 가져오기
    public function getSelltradingproducts($userid) {
        //$stmt = $this->conn->prepare("SELECT * FROM tradingproduct WHERE sell_user_id = ? ORDER BY created_at DESC");
        $stmt = $this->conn->prepare("SELECT * FROM tradingproduct LEFT JOIN product ON tradingproduct.product_id=product.product_id WHERE sell_user_id = ? ORDER BY tr_created_at DESC");
        $stmt->bind_param("i",$userid); //파라미터 값을 연결.
        $stmt->execute();
        $tasks = $stmt->get_result();
        $stmt->close();
        return $tasks;
    ...
    // 현재 거래중인 제품주에서 구매중인 정보들을 가져오기
    public function getBuytradingproducts($userid) {
        $stmt = $this->conn->prepare("SELECT * FROM tradingproduct LEFT JOIN product ON tradingproduct.product_id=product.product_id WHERE buy_user_id = ? ORDER BY tr_created_at DESC");
        $stmt->bind_param("i",$userid); //파라미터 값을 연결.
        $stmt->execute();
        $tasks = $stmt->get_result();
        $stmt->close();
    ...
    //해당 제품이 내가 거래 중인 제품인지 확인하는 기능.
    public function checkUserTrading($product_id,$buy_user_id) {
        $stmt = $this->conn->prepare("SELECT buy_user_id from tradingproduct WHERE product_id = ? AND buy_user_id = ?");
        $stmt->bind_param("ii", $product_id,$buy_user_id);
        $stmt->execute();
        $stmt->store_result();
    ...
    // 현재 거래정보 상태를 변경합니다
    public function updatetradingproducts($progress,$tradingproduct_id) {
        $stmt = $this->conn->prepare("UPDATE tradingproduct SET productprogress = ? WHERE tradingproduct_id = ?");
        $stmt->bind_param("ii",$progress,$tradingproduct_id); //파라미터 값을 연결.
        $stmt->execute();
        $tasks = $stmt->get_result();
        $stmt->close();
        return $tasks;
