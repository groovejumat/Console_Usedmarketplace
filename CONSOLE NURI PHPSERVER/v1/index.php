...
/**
 * 해당 user값을 가진 모든 판매거래중인 제품들을 가지고 옵니다.
 *  */
 $app->post('/getSelltradingproducts', function() {
     global $app; //post방식에서는 이를 기용한다. 이 구문이 무슨 뜻을 하는 것이길래 필요한 것일까?

     $response = array();
     $db = new DbHandler(); //여기서 getproductfunction을 실행 시킨다//

     $seller_name = $app->request->post('user_id'); //post로 username 값을 붙여 줍니다. 여기서 $app을 호출해 주어야 합니다.

     $result = $db->getSelltradingproducts($seller_name);

     $response["error"] = false;
     $response["tradingproducts"] = array();

     // pushing single chat room into array
     while ($tradingproduct = $result->fetch_assoc()) {
       $tmp = array();
       $tmp["tradingproduct_id"] = $tradingproduct["tradingproduct_id"];
       $tmp["sell_user_id"] = $tradingproduct["sell_user_id"];
       $tmp["buy_user_id"] = $tradingproduct["buy_user_id"];
       $tmp["product_id"] = $tradingproduct["product_id"];
       $tmp["productprogress"] = $tradingproduct["productprogress"];
       $tmp["productname"] = $tradingproduct["productname"];
       $tmp["imagelist"] = $tradingproduct["imagelist"];
       //$tmp["product_name"] = $tradingproduct["product_name"];
       //echo $chat_room["name"];
       $tmp["product_price"] = $tradingproduct["price"];
       $tmp["shippingInfo"] = $tradingproduct["shippingInfo"];
       $tmp["created_at"] = $tradingproduct["tr_created_at"];
       //$tmp=array_map("utf8_encode",$tmp)
       array_push($response["tradingproducts"], $tmp);
     }
 ...
 /**
  * 해당 user값을 가진 모든 구매거래중인 제품들을 가지고 옵니다.
  *  */
  $app->post('/getBuytradingproducts', function() {
      global $app; //post방식에서는 이를 기용한다. 이 구문이 무슨 뜻을 하는 것이길래 필요한 것일까?
      ...
      $result = $db->getBuytradingproducts($seller_name);
  ...
  //현재 제품이 거래 진행 중인 제품인지에 대해서 확인하는 api
  $app->post('/checkUserTrading', function() {
      global $app; //post방식에서는 이를 기용한다. 이 구문이 무슨 뜻을 하는 것이길래 필요한 것일까?
      ...
      $buy_user_id = $app->request->post('buy_user_id'); //post로 username 값을 붙여 줍니다. 여기서 $app을 호출해 주어야 합니다.

      $result = $db->checkUserTrading($product_id,$buy_user_id);

      $response["error"] = false;
      $response["tradingproducts"] = array();

      //$response = array_map("utf8_encode",$response);
      echoRespnse(200, $result); //값 반환
  });
   ...
   * 해당 user값을 가진 모든 구매거래중인 제품들을 가지고 옵니다.
   *  */
   $app->post('/getUpdatetradingprogress', function() {
       global $app; //post방식에서는 이를 기용한다. 이 구문이 무슨 뜻을 하는 것이길래 필요한 것일까?

       $response = array();
       $db = new DbHandler(); //여기서 getproductfunction을 실행 시킨다//

       $progress = $app->request->post('progress'); //post로 username 값을 붙여 줍니다. 여기서 $app을 호출해 주어야 합니다.
       $tradingproduct_id = $app->request->post('tradingproduct_id');

       $result = $db->updatetradingproducts($progress,$tradingproduct_id);

       $response["error"] = false;
       $response["tradingproducts"] = "업데이트 처리를 완료했음.";

       //global로 상태를 바뀌었을 때 이를 감지하는 처리를 하기 위함.
       ...
       $push->setData("데이터 변경 안내 메시지."); // 데이터 값을 세팅

       // echo json_encode($push->getPush());exit;

       // sending push message to a topic
       $gcm->sendToTopic('global', $push->getPush()); //topic에 해당하는 사람들에게맨 메시지를 보냄.
...
/**
 *post값으로 제품 진행 상황을 알려주기 위한
 *username의 이름을 가진, 채팅 룸을 구현한다.
 *  */
$app->post('/tradingproduct', function() { //채팅방의 이름은 유저의 닉네임으로 정해진다.
    global $app; //글로벌 변수 app?
    $db = new DbHandler();

    verifyRequiredParams(array('sell_user_id', 'buy_user_id', 'product_id','productprogress','shippinginfo')); // 바디값으로 필요한 값을 확인한다.

    $sell_user_id = $app->request->post('sell_user_id');
    $buy_user_id = $app->request->post('buy_user_id');
    $product_id = $app->request->post('product_id');
    $productprogress = $app->request->post('productprogress');
    $shippinginfo = $app->request->post('shippinginfo');


    $response = $db->addTradingproduct($sell_user_id, $buy_user_id, $product_id, $productprogress, $shippinginfo);
...
$app->post('/createchatroom', function() { //채팅방의 이름은 유저의 닉네임으로 정해진다.
    ...
    verifyRequiredParams(array('user_name', 'product_id', 'product_name','product_writer','writerid')); // 바디값으로 필요한 값을 확인한다.
    ...
    $writer_id = $app->request->post('writerid');


    $response = $db->createChatroom($user_name, $writer_id, $product_id, $product_name, $product_writer);
...
/**
 * Sending push notification to a single user //특정 대상에게 메시지를 보내줌.
...
$app->post('/users/:id/message', function($to_user_id) {
    ...
    if ($response['error'] == false) {
        ...
        $user = $db->getUser($to_user_id); //user정보를 id값으로 가지고 온다. 누구로 부터 왔는지 알려면 이렇게 하면 안된다.

        $data = array();
        $data['user'] = $fromuser;
        $data['message'] = $response['message']; //addMessage로 저장되어진 값의 메시지 정보를 가지고 온다.
        //$data['message'] = "거래 신청 메시지가 왔습니다.";
        $data['image'] = '';
        ...
        //$response['user'] = $user;
        $response['fromuser'] = $fromuser; //여기값을 notification이 받는다.
        $response['error'] = false;
    }
...
/**
 * 특정 대상id 값과 보내는 id값을 받아서 메시지(플래그에 따라 notification)을 보내준다.
 * * */
$app->post('/alerttowriter', function() {
    global $app;
    $db = new DbHandler();

    //verifyRequiredParams(array('message'));
    verifyRequiredParams(array('from_user_id','to_user_id', 'message'));

    $to_user_id = $app->request->post('to_user_id'); //보내는 대상 id
    $from_user_id = $app->request->post('from_user_id'); //보내는 사람 id
    $message = $app->request->post('message');
